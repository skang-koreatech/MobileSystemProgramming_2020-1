...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mX = findViewById(R.id.accelX);
        mY = findViewById(R.id.accelY);
        mZ = findViewById(R.id.accelZ);
        rmsTV = findViewById(R.id.rms);
        ...
        // graph 상에서 화면에 보여지는 x,y축에 대한 설정
        ...
        // UI 레이아웃에 정의된 GraphView 객체 참조 변수 생성
        graphRMS = findViewById(R.id.graphLinear);
        ...
        graphRMS.getViewport().setMinX(0);
        graphRMS.getViewport().setMaxX(60);
    ...
    public void onSensorChanged(SensorEvent event) {
        if(event.sensor.getType() == Sensor.TYPE_LINEAR_ACCELERATION) {
            ...
            double rms = Math.sqrt((values[0] * values[0] + values[1] * values[1] + values[2] * values[2])/3);
